...
class LLVMMCAPredictor(Predictor):
    ...
    def __init__(self, llvmmca_path, llvmmca_opts):
    ...
    def from_config(config):
        ...
        return LLVMMCAPredictor(llvmmca_path, llvmmca_opts)
    ...
    def evaluate(self, basic_block, disable_logging=False):
        ...
        start = timer()
        res = subprocess.run(cmd, input=asm_str, capture_output=True, encoding="latin1")
        end = timer()
        rt = end - start
